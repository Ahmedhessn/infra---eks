...
variable "environment" {
  type        = string
  description = "WHY: Isolate staging from other envs in names and state. WHAT: Environment name (staging)."
  default     = "staging"
}

variable "vpc_cidr" {
  type        = string
  description = "WHY: Each environment needs a different CIDR to avoid overlap. WHAT: VPC CIDR for staging."
...
variable "eks_node_instance_types" {
  type        = list(string)
  description = "Instance type(s) for the staging managed node group."
  default     = ["t3.medium"]
}

variable "eks_node_desired_size" {
  type        = number
  description = "Desired worker count for staging."
  default     = 2
}
...
variable "monitoring_instance_type" {
  type        = string
  description = "EC2 instance type for Prometheus + Grafana box (staging)."
  default     = "t3.small"
}
